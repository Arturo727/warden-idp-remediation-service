from fastapi.testclient import TestClient

from src.main import app

client = TestClient(app)


def test_prod_rollback_creates_approval():
    payload = {
        "project_id": "payments-api",
        "environment_id": "prod",
        "severity": "high",
        "signal": "P99 latency spiked to 4s after the 14:30 deploy",
        "context": {"last_deploy": "v2.3.1", "llm_mode": "with_llm"},
        "timestamp": "2024-04-03T14:45:00Z"
    }

    response = client.post("/webhooks/events", json=payload)
    assert response.status_code == 202
    body = response.json()
    assert body["status"] == "awaiting_approval"
    assert "approval_id" in body


def test_qa_restart_executes_automatically():
    payload = {
        "project_id": "orders-api",
        "environment_id": "qa",
        "severity": "medium",
        "signal": "pod crash detected with OOM",
        "context": {"llm_mode": "with_llm"},
        "timestamp": "2024-04-03T14:45:00Z"
    }

    response = client.post("/webhooks/events", json=payload)
    assert response.status_code == 202
    body = response.json()
    assert body["status"] == "executed"
    assert body["execution_result"]["status"] == "success"
